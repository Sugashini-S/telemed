...
export function AddPatientButton() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      options: { data: { name: form.name, role: "patient" } }
    });
    ...
    if (userError) { setError("Profile failed: " + userError.message); setLoading(false); return; }
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:"white",borderRadius:"16px",width:"100%",maxWidth:"460px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"24px 24px 16px",borderBottom:"1px solid #e5e7eb"}}>
              <h3 style={{margin:0,fontSize:"18px",fontWeight:"600"}}>Add New Patient</h3>
            ...
            <form onSubmit={handleSubmit} style={{padding:"24px",display:"flex",flexDirection:"column",gap:"16px"}}>
              ...
              {success && <div style={{padding:"12px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:"8px",color:"#16a34a",fontSize:"14px"}}>✅ Patient added successfully!</div>}
              ...
              <div>
                ...
                <input name="email" type="email" value={form.email} onChange={handleChange} required placeholder="patient@example.com"
                  style={{width:"100%",padding:"10px 12px",border:"1px solid #d1d5db",borderRadius:"8px",fontSize:"14px",boxSizing:"border-box"}} />
              </div>

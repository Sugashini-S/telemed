...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    if (roleData !== "admin") { await supabase.auth.signOut(); setError("Access denied. Not an admin."); setLoading(false); return; }
    router.push("/admin");
    router.refresh();
  };
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#111827"}}>
      <div style={{background:"#1f2937",padding:"2rem",borderRadius:"1rem",width:"100%",maxWidth:"400px",margin:"0 16px"}}>
        ...
        <p style={{color:"#9ca3af",textAlign:"center",marginBottom:"1.5rem",fontSize:"0.875rem"}}>TN Clinic Booker — Admin Panel</p>
        ...
        <form onSubmit={handleLogin}>
          <div style={{marginBottom:"1rem"}}>
            ...
            <input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} required placeholder="admin@example.com"
              style={{width:"100%",padding:"0.75rem",background:"#374151",border:"1px solid #4b5563",borderRadius:"0.5rem",color:"white",fontSize:"0.875rem",boxSizing:"border-box"}} />
          </div>
          ...
          <button type="submit" disabled={loading}
            style={{width:"100%",padding:"0.75rem",background:loading?"#15803d":"#16a34a",color:"white",border:"none",borderRadius:"0.5rem",fontWeight:"bold",fontSize:"1rem",cursor:loading?"not-allowed":"pointer"}}>

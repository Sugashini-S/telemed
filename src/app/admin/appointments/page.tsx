  ...
  const appointments = await Promise.all(
    (data || []).map(async (a) => {
      const [{ data: patient }, { data: doctor }] = await Promise.all([
        ...
        supabase.from("doctors").select("specialization, user_id").eq("id", a.doctor_id).single(),
      ]);
      const { data: docUser } = doctor?.user_id
        ? await supabase.from("users").select("name").eq("id", doctor.user_id).single()
        : { data: null };
      return {
        id: a.id,
        status: a.status,
    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {appointments.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-gray-400">No appointments found</td></tr>}
            {appointments.map((a) => (
              <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="py-3 px-2 text-gray-500">{new Date(a.created_at).toLocaleDateString()}</td>
                <td className="py-3 px-2">
